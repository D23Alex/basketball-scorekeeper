...
public class StatController {
    ...
    Optional<PlayerGameStatLine> playerGameStatLine(@PathVariable Long playerId, @PathVariable Long gameId) {
        ...
        return Optional.of(new PlayerGameStatLine(player.get(), databaseQueries.gameEventsByGameId(gameId)));
    }
    ...
    Optional<List<PlayerGameStatLine>> playerGameStatLine(@PathVariable Long gameId) {
        ...
        var players = gameEventLog.getLineupOccurrences().stream().map(LineupOccurrence::getPlayer);
        return Optional.of(players.map(player -> new PlayerGameStatLine(player, gameEventLog)).toList());
    }
}
